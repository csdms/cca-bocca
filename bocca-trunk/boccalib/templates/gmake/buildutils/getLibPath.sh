...
for symbol in $symbols; do
	...
	if [ "$client_or_server" = "server" ]; then
        ...
        if test -f "$topdir/sidl/$symbol.sidl"; then
            isenum=`grep -E '   enum ' $topdir/sidl/$symbol.sidl | wc -l | sed -e 's| ||g'`
            isenum=${isenum%% *}
        else 
            isenum="0"
        fi
		...
		if [ "$language" = "none" ]; then
            ...
            if [ "x$isenum" = "x1" ]; then
                # This is an enum, which has no C module source
			    libname="$libdir/`echo $symbol | sed -e 's|\.|/|g'`.pystamp"
            else
