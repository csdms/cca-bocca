#!/bin/bash
usage () {
 	echo "$0: usage: <mode> <libpath> <palette-alias> <language> [dynamic <scope> <resolution>] <babelClassName space separated list>" > /dev/stderr
	...
	echo "            language is the implementation language (c cxx f77 f90 python)" > /dev/stderr
	echo "            babelClassName is a space separated list of the full dot-qualified" > /dev/stderr
	...
	echo "e.g.: $0 scl /somewhere/lib/libComponent3.a Comp3 cxx test3.Component1" > /dev/stderr
...
for className in $classes; do
    ...
    if [ "x$language" = "xjava" ] ; then
      echo "    <class name=\"$className\" desc=\"java/impl\" />"
    fi  
done
cat << __EOF2
  </library>
