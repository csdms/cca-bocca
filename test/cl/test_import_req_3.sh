...
if ! test -d "$2/iir3/myproj/BOCCA"; then
...
if test "$?" != "0"; then 
	echo "FAIL: combined requires, import not working on interface"
	exit 1; 
fi
...
if grep getSubscriptionName $dpsidl/$ENAME.Subscription2.sidl ; then
	...
	echo "FAIL: bocca combined requires/import failed silently on interface."
	exit 1;
fi

cat << EOF > in.scijump.Subscription.sidl
package scijump version 0.0 {

    class Subscription implements-all gob.ccb.Subscription
    {
        void initialize(in string subscriptionName, in gov.cca.TypeMap tm);             

        
        void processEvents(in array<gob.ccb.Event, 1> eventList);               

    }
}
EOF

$1 create package $INAME --version=0.0
if test "$?" != "0"; then 
	echo "FAIL: create package $INAME?"
	exit 1; 
fi
$1 create class $INAME.Subscription --implements=$PNAME.Subscription --requires=$PNAME.Event
if test "$?" != "0"; then 
	echo "FAIL: create class $INAME.Subscription?"
	exit 1; 
fi
$1 change class $INAME.Subscription \
--import-sidl=$INAME.Subscription@./in.$INAME.Subscription.sidl
if test "$?" != "0"; then 
	echo "FAIL: change class $INAME.Subscription?"
	exit 1;
fi

$1 create class $INAME.Subscription2 --implements=$PNAME.Subscription --requires=$PNAME.Event \
--import-sidl=$INAME.Subscription@./in.$INAME.Subscription.sidl
if test "$?" != "0"; then 
	echo "FAIL: create class $INAME.Subscription2?"
	exit 1; 
fi
