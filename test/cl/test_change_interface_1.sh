#!/bin/sh
# test interface change, importing SIDL w/build merge
topdir=`pwd`
cd $2
if ! test "x$?" = "x0" ; then
...
if ! test -d "$2/chiface1/from/BOCCA"; then
...
if ! test -d "$2/chiface1/to/BOCCA"; then
...
$1 create interface foobar --import-sidl=$2/chiface1/from/ports/sidl/mypkg.iface.sidl
if ! test "x$?" = "x0"; then
    echo "Failed to import sidl from $2/chiface1/from/ports/sidl/mypkg.iface.sidl"
    echo "FAIL"
    exit 1
fi
