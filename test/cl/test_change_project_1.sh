#!/bin/sh
# test project change, importing SIDL w/build merge
topdir=`pwd`
cd $2
if ! test "x$?" = "x0" ; then
...
if ! test -d "$2/chproj1/from/BOCCA"; then
...
if ! test -d "$2/chproj1/to/BOCCA"; then
...
$1 change project --import-sidl=$2/chproj1/from/ports/sidl/mypkg.iface.sidl
if ! test "x$?" = "x0"; then
    echo "Failed to import sidl from $2/chproj1/from/ports/sidl/mypkg.iface.sidl"
    echo "FAIL"
    exit 1
fi

# Import from/components/sidl/mypkg.foo.sidl
$1 change project --import-sidl=$2/chproj1/from/components/sidl/mypkg.foo.sidl
if ! test "x$?" = "x0"; then
    echo "Failed to import sidl from $2/chproj1/from/components/sidl/mypkg.foo.sidl"
    echo "FAIL"
    exit 1
fi
